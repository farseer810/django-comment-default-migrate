...
from django_comment_default_migrate.db_comments import get_migration_class_from_engine
from django_comment_default_migrate.utils import get_migrations_app_models
from tests.models import CommentModel


class TestDjangoCommentDefaultMigration(TestCase):
    def test_get_migration_class_from_engine(self):
        engine_migration_class_mapping = {
            "django.db.backends.mysql": "django_comment_default_migrate.backends.mysql.CommentDefaultMigration",
            "django.db.backends.postgresql": "django_comment_default_migrate.backends.postgresql.CommentDefaultMigration",
            "django.db.backends.postgresql_psycopg2": "django_comment_default_migrate.backends.postgresql.CommentDefaultMigration",
            "mssql": "django_comment_default_migrate.backends.mssql.CommentDefaultMigration",
        }
    ...
    @override_settings(
        DCDM_COMMENT_KEY="verbose_name", DCDM_TABLE_COMMENT_KEY="verbose_name_plural"
    )
    ...
    @override_settings(
        DCDM_BACKEND={
            "new-engine": "django_comment_default_migrate.backends.mssql.CommentDefaultMigration"
        }
    )
...
class TestCommand(TestCase):
    ...
    def test_migrate_command_without_app_label(self):
        out = io.StringIO()
        management.call_command("migratecommentdefault", stdout=out)
